...
def load_and_cache_examples_subdict(args, tokenizer, labels, pad_token_label_id, mode):
    """
        读取5个subdict标注的数据
    """
    if args.local_rank not in [-1, 0] and not evaluate:
    ...
    for idx in range(1, args.num_head):
        cached_features_file = os.path.join(
            args.data_dir,
            "cached_{}_{}_{}_{}".format(
                mode, list(filter(None, args.model_name_or_path.split("/"))).pop(), str(args.max_seq_length), idx-1
            ),
        )
        cached_features_files.append(cached_features_file)
    
    if os.path.exists(cached_features_files[0]) and not args.overwrite_cache:
        logger.info("Loading features from cached file %s", cached_features_files[0])
        featureses = []
        ...
        featureses = []
        for idx in range(1, args.num_head):
            examples = read_examples_from_file(args.data_dir, mode+"{}".format(idx-1))
            features = convert_examples_to_features(
                examples,
                labels,
                args.max_seq_length,
            ...
            featureses.append(features)
        if args.local_rank in [-1, 0]:
            logger.info("Saving features into cached file %s", cached_features_files[0])
            for idx, cached_features_file in enumerate(cached_features_files):
                torch.save(featureses[idx], cached_features_file)
...
class OurRandomSampler(Sampler):
    ...
    def __init__(self, data_source, shuffle_list, master, replacement=False, num_samples=None):
