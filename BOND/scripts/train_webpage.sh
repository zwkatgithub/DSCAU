...
DATA_ROOT=$PROJECT_ROOT/ourdataset/webpage_distant/
...
# output
SAVED_DIR=$PROJECT_ROOT/models/webpage/self_training/${MODEL_TYPE}_reinit${REINIT}_begin${BEGIN_STEP}_period${PERIOD}_${LABEL_MODE}_hp${HP_LABEL}_${EPOCH}_${LR}/
...
CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=$GPUID nohup python3 run_self_training_ner_subdict.py --data_dir $DATA_ROOT \
  ...
  --lamb $LAMB --num 1 > $LOGS_DIR/webpage.log &