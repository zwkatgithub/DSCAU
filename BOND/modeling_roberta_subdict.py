...
class RobertaForTokenClassification_v2(BertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if rm_u is not None:
                rm_u = rm_u.view(-1)[active_loss]
            if add_u is not None:
                add_u = add_u.view(-1)[active_loss]
            ...
            if labels.shape == logits.shape:
                ...
                if attention_mask is not None or label_mask is not None:
                    ...
                    if rm_u is not None:
                        active_labels[rm_u] = 0
                    loss_rm = loss_fct(active_logits, active_labels)
                    if add_u is not None and add_u.sum()!=0:
                        loss_add = loss_fct(active_logits[add_u], active_labels[add_u])
                    else:
                        loss_add = torch.FloatTensor([0]).to(active_labels.device)
                    
                    loss = loss_rm + loss_add
                else:
