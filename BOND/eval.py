...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, best, mode, prefix="", verbose=True):
    ...
    tps, pres, recs = defaultdict(int), defaultdict(int), defaultdict(int)
    ...
    for ground_truth_id,predicted_id in zip(out_id_list,preds_id_list):
        ...
        for chunk in list(lab_pred_chunks & lab_chunks):
            tps[chunk[0]] += 1
    ...
    for t in recs.keys():
        precisions[t] = tps[t] / pres[t] if pres[t]!=0 else 0
        recalls[t] = tps[t] / recs[t] if recs[t]!=0 else 0
        f1s[t] = 2*precisions[t]*recalls[t] / (precisions[t]+recalls[t]) if (precisions[t]+recalls[t])!=0 else 0

    is_updated = False
    if new_F > best[2]:
        best = [p, r, new_F, precisions, recalls, f1s]
        is_updated = True
    ...
    for t in recs.keys():
        ...
        results.update({t+"_pre":precisions[t]})
        results.update({t+"_rec":recalls[t]})
        results.update({t+"_f1": f1s[t]})

        results.update({"best_"+t+"_pre": best[3][t]})
        results.update({"best_"+t+"_rec": best[4][t]})
        results.update({"best_"+t+"_f1": best[5][t]})
